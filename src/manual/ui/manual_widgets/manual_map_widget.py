...
class ManualMapWidget(QWidget):
    def __init__(self, tree_widget: "ManualTreeWidget", top_item_index: int, parent=None) -> None:
        ...
        ManualInitProvider.apply_tab_texts(self.objectName(), self.manual_map_tab_widget, tab_widgets, self)
        ManualInitProvider.initialize_text_edits(tab_widgets, self)

    def create_gui(self) -> QLayout:
        main_layout = QVBoxLayout()
        self.manual_map_tab_widget = QTabWidget()
        self.manual_map_tab_widget.currentChanged.connect(self.set_tree_item)
    ...
    def set_tree_item(self) -> None:
        ...
        if current_item is top_item:
            return
        tab_index = self.manual_map_tab_widget.currentIndex()
