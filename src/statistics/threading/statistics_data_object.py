...
class StatisticsDataObject(QObject):
    ...
    def get_statistics_data(self):
        ...
        try:
            ...
            results["completion"]["mandatory"] = self.query_provider.get_completion_data(db_connection, "mandatory")
            results["completion"]["work"] = self.query_provider.get_completion_data(db_connection, "work")
            results["completion"]["social"] = self.query_provider.get_completion_data(db_connection, "social")
            results["completion"]["detail"] = self.query_provider.get_completion_data(db_connection, "detail")
            results["completion"]["info"] = self.query_provider.get_completion_data(db_connection, "info")
            self.data_ready.emit(results)
            self.finished.emit()
        except Exception:
            self.data_ready.emit(results)
